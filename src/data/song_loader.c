...
{
        if (strncmp(songdata->file_path, "youtube://", 10) == 0) {
...
void load_meta_data(SongData *songdata)
{
        ...
        if (strncmp(songdata->file_path, "youtube://", 10) == 0) {
            char *id_start = strchr(songdata->file_path, '[');
            char *id_end = strchr(songdata->file_path, ']');
            char *dur_start = strstr(songdata->file_path, "(dur:");
            if (id_start && id_end) {
                *id_end = '\0';
                char *title = id_end + 1;
                if (dur_start) *dur_start = '\0'; // Temporarily cut at duration
                strncpy(songdata->metadata->title, title, METADATA_MAX_LENGTH - 1);
                if (dur_start) *dur_start = '('; // Restore
                strncpy(songdata->metadata->artist, "YouTube", METADATA_MAX_LENGTH - 1);
                strncpy(songdata->metadata->album, "YouTube", METADATA_MAX_LENGTH - 1);
                *id_end = ']'; // Restore it
            }
            if (dur_start) {
                int m = 0, s = 0, h = 0;
                if (sscanf(dur_start, "(dur:%d:%d:%d)", &h, &m, &s) == 3)
                    songdata->duration = h * 3600 + m * 60 + s;
                else if (sscanf(dur_start, "(dur:%d:%d)", &m, &s) == 2)
                    songdata->duration = m * 60 + s;
            }
            return;
        }
