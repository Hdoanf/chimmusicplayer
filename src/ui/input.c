...
struct Event map_tb_key_to_event(struct tb_event *ev)
{
        ...
        if (state->currentView == SEARCH_VIEW || state->currentView == YOUTUBE_SEARCH_VIEW) {
                ...
                // Alt+Enter for YouTube search execute (must have ALT mod)
                else if ((ev->key == TB_KEY_ENTER || ev->ch == '\r') && (ev->mod & TB_MOD_ALT) && state->currentView == YOUTUBE_SEARCH_VIEW) {
                        event.type = EVENT_YOUTUBE_SEARCH_EXECUTE;
                }
                // Plain Enter should do nothing here, letting it fall through to key_bindings
                else if ((ev->key == TB_KEY_ENTER || ev->ch == '\r') && state->currentView == YOUTUBE_SEARCH_VIEW) {
...
void handle_event(struct Event *event)
{
        ...
        case EVENT_SHOWSEARCH:
                ...
                break;
        case EVENT_SHOWYOUTUBESEARCH:
                if (state->currentView == YOUTUBE_SEARCH_VIEW) {
                        // Already in view, treat as execute search
