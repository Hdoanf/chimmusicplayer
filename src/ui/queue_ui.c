...
void view_enqueue(bool play_immediately)
{
        ...
        if (state->currentView == YOUTUBE_SEARCH_VIEW) {
                YoutubeResult *res = get_youtube_result(chosen_youtube_result_row);
                if (res) {
                        set_error_message("Preparing YouTube song...");
                        trigger_refresh();
                        print_player(NULL, 0.0);
                        fflush(stdout);

                        char yt_path[512];
                        snprintf(yt_path, sizeof(yt_path), "youtube://[%s]%s(dur:%s)", res->video_id, res->title, res->duration);
                        Node *new_node = NULL;
                        create_node(&new_node, yt_path, increment_node_id());
                        if (new_node) {
                                add_to_list(playlist, new_node);
                                first_enqueued_node = new_node;
                                deep_copy_play_list_onto_list(playlist, &unshuffled_playlist);
                                if (playlist->count == 1 || is_stopped()) {
