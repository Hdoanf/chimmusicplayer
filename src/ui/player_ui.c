...
int print_logo(SongData *song_data, UISettings *ui, int row, int col)
{
        ...
        if (ui->hideLogo) {
                ...
                height = print_logo_art(row, 0, ui, true, false, true);
        }
...
void calc_indent(SongData *songdata)
{
        ...
        if (state->currentView != TRACK_VIEW || (state->currentView == TRACK_VIEW && songdata == NULL)) {
...
void switch_to_next_view(void)
{
        ...
        case TRACK_VIEW:
                ...
                break;
        case SEARCH_VIEW:
                state->currentView = YOUTUBE_SEARCH_VIEW;
                break;
        case YOUTUBE_SEARCH_VIEW:
...
void scroll_next(void)
{
        ...
        } else if (state->currentView == YOUTUBE_SEARCH_VIEW) {
                chosen_youtube_result_row++;
                if (chosen_youtube_result_row >= get_youtube_results_count())
                        chosen_youtube_result_row = get_youtube_results_count() - 1;
                trigger_refresh();
        }
}
...
void scroll_prev(void)
{
        ...
        if (state->currentView == PLAYLIST_VIEW) {
                ...
                chosen_youtube_result_row--;
                if (chosen_youtube_result_row < 0)
                        chosen_youtube_result_row = 0;
                trigger_refresh();
        }
}
...
void show_youtube_search(SongData *song_data, int *chosen_row)
{
        int term_w, term_h;
        get_term_size(&term_w, &term_h);
        max_search_list_size = term_h - 2;
        ...
        if (term_w > col + 38 && !ui->hideHelp) {
                printf("\033[%d;%dH", ++row, col + 2);
                printf(_(" Select:↑/↓."));
                printf(_(" Search:F5/Alt+Enter."));
                printf(_(" Enqueue:Enter."));
                printf("\033[%d;%dH", ++row, col + 2);
                clear_rest_of_line();
        ...
        display_youtube_search(row, col, max_search_list_size, chosen_row);
        calc_and_print_last_row_and_error_row();
}
...
int print_player(SongData *songdata, double elapsed_seconds)
{
        ...
        if (state->currentView == HELP_VIEW && shouldRefresh) {
                ...
                fflush(stdout);
        } else if (state->currentView == YOUTUBE_SEARCH_VIEW && shouldRefresh) {
                cancel_refresh();
                show_youtube_search(songdata, &chosen_youtube_result_row);
                fflush(stdout);
