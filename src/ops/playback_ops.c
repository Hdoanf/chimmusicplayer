...
int load_decoder(SongData *song_data, bool *song_data_deleted)
{
        ...
        if (song_data != NULL) {
                *song_data_deleted = false;
                ...
                if (strncmp(song_data->file_path, "youtube://", 10) == 0) {
                    char *id_start = strchr(song_data->file_path, '[');
                    char *id_end = strchr(song_data->file_path, ']');
                    if (id_start && id_end) {
                        ...
                        *id_end = ']';
                        ...
                        if (stream_url) {
                            c_strcpy(song_data->file_path, stream_url, sizeof(song_data->file_path));
                            
                            // Reload real metadata
                            load_meta_data(song_data);
                            load_color(song_data);
...
void *song_data_reader_thread(void *arg)
{
        ...
        SongData *songdata = (exists_file(filepath) >= 0 || strncmp(filepath, "youtube://", 10) == 0) ? load_song_data(filepath) : NULL;
