...
bool valid_file_path(char *file_path)
{
        ...
        if (strncmp(file_path, "youtube://", 10) == 0 ||
            strncmp(file_path, "http", 4) == 0)
                return true;
...
int pb_switch_audio_implementation(void)
{
        ...
        if (strncmp(audio_data.pUserData->current_song_data->file_path, "youtube://", 10) == 0) {
            char *id_start = strchr(audio_data.pUserData->current_song_data->file_path, '[');
            char *id_end = strchr(audio_data.pUserData->current_song_data->file_path, ']');
            if (id_start && id_end) {
                set_error_message("Downloading from YouTube...");
                trigger_refresh();
                
                *id_end = '\0';
                char video_id[32];
                strncpy(video_id, id_start + 1, 31);
                video_id[31] = '\0';
                
                char title[256] = {0};
                char *title_ptr = id_end + 1;
                char *dur_marker = strstr(title_ptr, "(dur:");
                if (dur_marker) {
                    int len = dur_marker - title_ptr;
                    if (len > 255) len = 255;
                    strncpy(title, title_ptr, len);
                } else {
                    strncpy(title, title_ptr, 255);
                }

                *id_end = ']';
                char *stream_url = youtube_get_stream_url(video_id, title);
                if (stream_url) {
                    c_strcpy(audio_data.pUserData->current_song_data->file_path, stream_url, sizeof(audio_data.pUserData->current_song_data->file_path));
                    
                    // Reload real metadata from the newly downloaded file
                    extern void load_meta_data(SongData *songdata);
                    extern int load_color(SongData *songdata);
                    load_meta_data(audio_data.pUserData->current_song_data);
                    load_color(audio_data.pUserData->current_song_data);
